...
class Lreceipt {
	...
	public function get_single_receipt($transaction_id)
	{
		$CI =& get_instance();
		$CI->load->model('receipts');
		$CI->load->library('occational');
		$receipt_list = $CI->receipts->retreive_single_receipt($transaction_id);	
		if(!empty($receipt_list)){
			$i= 0;
			foreach($receipt_list as $k=>$v){$i++;
			   $receipt_list[$k]['sl']=$i;
		...
		$receiptList = $CI->parser->parse('receipt/single_receipt',$data,true);
		return $receiptList;
	}

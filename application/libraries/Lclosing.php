...
class Lclosing {
	...
	public function get_daily_closing_view()
	{
		...
		$draw_view = $CI->parser->parse('closing/drawing_form',$draw_data,true);
		
		$transaction_data = array(
				'title' => 'Daily Closing'
			);
		$transaction_view = $CI->parser->parse('closing/expence_form',$transaction_data,true);
		...
		$bank_view = $CI->parser->parse('closing/banking_form',$bank_data,true);
		
		
		//Get Daily Drawing Amount			 
		$closing_amount =$CI->closings->get_last_closing_amount();		
		...
		//Get Daily Drawing Amount			 
		$draw_amount =$CI->closings->get_todays_draw_amount();		
		$total_draw_amount = 0;
		...
		$expense_amount =$CI->closings->get_todays_expense_amount();		
	...
	public function daily_closing_list($limit,$page,$links)
	{
		$CI =& get_instance();
		$CI->load->model('closings');
		$CI->load->library('occational');
		$daily_closing_data = $CI->closings->get_closing_report($limit,$page);
		...
		$reportList = $CI->parser->parse('closing/closing_report',$data,true);
		return $reportList;
	}
	...
	public function get_date_wise_closing_reports( $from_date,$to_date )
	{
		...
		$CI->load->model('closings');
		$CI->load->library('occational');
		$daily_closing_data = $CI->closings->get_date_wise_closing_report( $from_date,$to_date );
